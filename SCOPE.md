...
*Scope change 2026-09-19 (remote-download pipeline): music acquisition is now in
scope in one specific form — `musik fetch` / `musik bot` turn Spotify/YouTube
links into downloaded `.m4a` files (pinned spotDL/SomeDL, no re-encoding of the
library) which then run through the normal tagging chain. Also in scope: one
